...
const initialData: CertificateRow[] = [
  {
    id: '1',
    sNo: 1,
    certificateNo: 'CERT123456',
    date: '2023-10-25',
    certificateType: 'Origin Certificate',
    commissionerate: 'Mumbai Zone I',
    range: 'Range-I',
    division: 'Division-A',
    isSelected: false
  },
  {
    id: '2',
    sNo: 2,
    certificateNo: 'CERT789012',
    date: '2023-11-15',
    certificateType: 'Quality Certificate',
    commissionerate: 'Delhi Zone II',
    range: 'Range-II',
    division: 'Division-B',
    isSelected: false
  }
...
export default function CertificateDetails() {
  const [data, setData] = useState<CertificateRow[]>(initialData);
  ...
  const [columnWidths] = useState<number[]>([
    150, // Certificate No
    120, // Date
    180, // Certificate Type
    180, // Commissionerate
    150, // Range
    150, // Division (Flex)
  ]);

  const [editingCell, setEditingCell] = useState<{ rowId: string, columnKey: keyof CertificateRow } | null>(null);
  ...
  const columns: { key: keyof CertificateRow | 'checkbox' | 'sNo' | 'actions'; label: string; width: number; required?: boolean; fixed?: 'left' | 'right'; left?: number; right?: number }[] = [
    { key: 'checkbox', label: '', width: checkboxWidth, fixed: 'left', left: 0 },
    { key: 'sNo', label: 'S.No', width: snoWidth, fixed: 'left', left: checkboxWidth },
    { key: 'certificateNo', label: 'Certificate No', width: columnWidths[0] },
    { key: 'date', label: 'Date', width: columnWidths[1] },
    { key: 'certificateType', label: 'Certificate Type', width: columnWidths[2] },
    { key: 'commissionerate', label: 'Commissionerate', width: columnWidths[3] },
    { key: 'range', label: 'Range', width: columnWidths[4] },
    { key: 'division', label: 'Division', width: columnWidths[5] },
  ...
  const editableKeys: (keyof CertificateRow)[] = [
    'certificateNo', 'date', 'certificateType', 'commissionerate', 'range', 'division'
  ];

  const navigableColumns = columns.filter(c => editableKeys.includes(c.key as keyof CertificateRow));
  ...
  // Editing Logic
  const startEdit = (rowId: string, columnKey: keyof CertificateRow, value: any) => {
  ...
  const navigateTo = (rowIndex: number, colIndex: number) => {
    ...
    const cellId = `cert-cell-${r}-${colKey}`;
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (col && data[rowIndex]) startEdit(data[rowIndex].id, col.key as keyof CertificateRow, data[rowIndex][col.key as keyof CertificateRow]);
        break;
      case 'Tab':
        e.preventDefault();
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white font-['Inter',sans-serif]" ref={tableRef} onMouseLeave={() => !focusedCellId && setHighlightPosition(null)}>
      ...
      <div className="flex items-center justify-center h-[40px] bg-[#DDE3F2] border-b border-[#d0d5e3]">
        <h2 className="text-[16px] font-semibold text-[#050e25]">Certificate Details</h2>
      </div>
            ...
            <div className="flex h-[34px] border-b border-[#d0d5e3] sticky top-0 z-20 bg-[#ebeef7]">
              {columns.map((col, idx) => (
                <div
                  ...
                  style={{
                    width: col.key === 'division' ? 'auto' : col.width,
                    flex: col.key === 'division' ? 1 : 'none',
                    minWidth: col.key === 'division' ? col.width : undefined,
            ...
            {data.map((row, rIdx) => (
              <div key={row.id} className="flex h-[34px] group relative">
                {columns.map((col, cIdx) => {
                  ...
                  const cellId = `cert-cell-${rIdx}-${col.key}`;
                  ...
                  return (
                    <div 
                      ...
                      onDoubleClick={() => startEdit(row.id, col.key as keyof CertificateRow, row[col.key as keyof CertificateRow])}
                    ...
                    >
                      {isEditing ? (
                        ...
                        <>
                          <span className="truncate w-full">{row[col.key as keyof CertificateRow] as string}</span>
                        </>
                      )}
                    </div>
                  );
          ...
          {/* Scrollable footer content synced with main table */}
           <div ref={footerScrollRef} className={SCROLLBAR_CLASSES.HIDDEN} onScroll={handleFooterScroll}>
             <div className="flex h-[34px]">
               ...
               {scrollableColumns.filter(c => c.key !== 'certificateNo').map((col, idx) => (
                 <div 
                   key={col.key} 
                   style={{ 
                     width: col.key === 'division' ? 'auto' : col.width,
