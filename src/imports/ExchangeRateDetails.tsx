...
const initialData: ExchangeRateRow[] = [
  {
    id: '1',
    sNo: 1,
    currencyCode: 'USD',
    ...
    standardCurrency: 'Yes',
    bankName: 'SBI',
    certificateNo: 'CERT001',
    certificateDate: '2023-10-20',
    isSelected: false
  },
  {
    id: '2',
...
export default function ExchangeRateDetails() {
  const [data, setData] = useState<ExchangeRateRow[]>(initialData);
  ...
  const [columnWidths] = useState<number[]>([
    ...
    100, // Rate
    120, // Effective Dt.
    140, // Standard Currency
    150, // Bank Name
    150, // Certificate No
    150, // Certificate Date (Flex)
  ]);

  const [editingCell, setEditingCell] = useState<{ rowId: string, columnKey: keyof ExchangeRateRow } | null>(null);
  ...
  const columns: { key: keyof ExchangeRateRow | 'checkbox' | 'sNo' | 'actions'; label: string; width: number; required?: boolean; fixed?: 'left' | 'right'; left?: number; right?: number }[] = [
    { key: 'checkbox', label: '', width: checkboxWidth, fixed: 'left', left: 0 },
    { key: 'sNo', label: 'S.No', width: snoWidth, fixed: 'left', left: checkboxWidth },
    { key: 'currencyCode', label: 'Currency Code', width: columnWidths[0], required: true },
    { key: 'unitInRs', label: 'Unit in Rs', width: columnWidths[1] },
    { key: 'rate', label: 'Rate', width: columnWidths[2], required: true },
    { key: 'effectiveDate', label: 'Effective Dt.', width: columnWidths[3] },
    { key: 'standardCurrency', label: 'Standard Currency', width: columnWidths[4] },
    { key: 'bankName', label: 'Bank Name', width: columnWidths[5] },
    { key: 'certificateNo', label: 'Certificate No', width: columnWidths[6] },
    { key: 'certificateDate', label: 'Certificate Date', width: columnWidths[7] },
  ...
  const editableKeys: (keyof ExchangeRateRow)[] = [
    'currencyCode', 'unitInRs', 'rate', 'effectiveDate', 'standardCurrency', 'bankName', 'certificateNo', 'certificateDate'
  ];

  const navigableColumns = columns.filter(c => editableKeys.includes(c.key as keyof ExchangeRateRow));
  ...
  // Editing Logic
  const startEdit = (rowId: string, columnKey: keyof ExchangeRateRow, value: any) => {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (col && data[rowIndex]) startEdit(data[rowIndex].id, col.key as keyof ExchangeRateRow, data[rowIndex][col.key as keyof ExchangeRateRow]);
        break;
      case 'Tab':
        e.preventDefault();
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white font-['Inter',sans-serif]" ref={tableRef} onMouseLeave={() => !focusedCellId && setHighlightPosition(null)}>
      ...
      <div className="flex items-center justify-center h-[40px] bg-[#DDE3F2] border-b border-[#d0d5e3]">
        <h2 className="text-[16px] font-semibold text-[#050e25]">Exchange Rate</h2>
      </div>
            ...
            <div className="flex h-[34px] border-b border-[#d0d5e3] sticky top-0 z-20 bg-[#ebeef7]">
              {columns.map((col, idx) => (
                <div
                  ...
                  style={{
                    width: col.key === 'certificateDate' ? 'auto' : col.width,
                    flex: col.key === 'certificateDate' ? 1 : 'none',
                    minWidth: col.key === 'certificateDate' ? col.width : undefined,
            ...
            {data.map((row, rIdx) => (
              <div key={row.id} className="flex h-[34px] group relative">
                {columns.map((col, cIdx) => {
                  ...
                  const cellId = `ex-cell-${rIdx}-${col.key}`;
                  ...
                  return (
                    <div 
                      ...
                      onDoubleClick={() => startEdit(row.id, col.key as keyof ExchangeRateRow, row[col.key as keyof ExchangeRateRow])}
                    ...
                    >
                      {isEditing ? (
                        ...
                        <>
                          <span className="truncate w-full">{row[col.key as keyof ExchangeRateRow] as string}</span>
                        </>
                      )}
                    </div>
                  );
          ...
          {/* Scrollable footer content synced with main table */}
           <div ref={footerScrollRef} className={SCROLLBAR_CLASSES.HIDDEN} onScroll={handleFooterScroll}>
             <div className="flex h-[34px]">
               ...
               {scrollableColumns.filter(c => c.key !== 'currencyCode').map((col, idx) => (
                 <div 
                   key={col.key} 
                   style={{ 
                     width: col.key === 'certificateDate' ? 'auto' : col.width,
