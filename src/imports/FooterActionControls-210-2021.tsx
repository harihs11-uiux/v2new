import svgPaths from "./svg-k2ucqaucqq";
...
function Button({ selectedCount }: { selectedCount: number }) {
  return (
    <div
    ...
    >
      ...
      <div className="flex flex-col font-['Inter:Semi_Bold',_sans-serif] font-semibold justify-center not-italic relative shrink-0 text-[#3874ff] text-[14px] text-left text-nowrap">
        <p className="block leading-[18px] whitespace-pre">{selectedCount} items selected</p>
      </div>
    </div>
  );
}
...
function Button1({ onClick }: { onClick: () => void }) {
  return (
    <div
      className="box-border content-stretch flex flex-row gap-1 items-center justify-center leading-[0] px-2 py-1 relative rounded shrink-0 cursor-pointer hover:opacity-80"
      data-name="Button"
      onClick={onClick}
    >
      <IconLeft1 />
      <div className="flex flex-col font-['Inter:Semi_Bold',_sans-serif] font-semibold justify-center not-italic relative shrink-0 text-[#cf3b3b] text-[14px] text-left text-nowrap">
        <p className="block leading-[18px] whitespace-pre">Cancel Job</p>
      </div>
    </div>
  );
}

interface FooterActionControlsProps {
  selectedCount: number;
  onCancel: () => void;
}

export default function FooterActionControls({ selectedCount, onCancel }: FooterActionControlsProps) {
  ...
  return (
    <div
      className="bg-[#050e25] box-border content-stretch flex flex-row gap-2 items-center justify-start p-0 relative rounded-lg w-fit h-fit px-2 py-2"
      data-name="Footer action Controls"
    >
      <Button selectedCount={selectedCount} />
          ...
          <div className="h-0 relative w-5" data-name="Seperator">
            <div
              ...
              style={
                {
                  "--stroke-0":
                    "rgba(80.00000283122063, 87.00000241398811, 103.0000014603138, 1)",
