...
import { MoreVertical, Plus, AlertCircle } from 'lucide-react';
...
export interface HssRow {
  ...
  country: string;
  adCode: string;
  precedingLevel: string;
  isSelected: boolean;
}

const initialData: HssRow[] = [
  {
    id: '1',
    sNo: 1,
    importerName: 'SUMITOMO ELECTRIC HARDMETAL INDIA PVT LTD',
    importerId: 'NF00031274',
    iec: '0500002893',
    branch: '0',
    address: 'Unit No. 1002, 10th Floor, Tower B',
    city: 'GURGAON',
    state: 'HARYANA',
    pincode: '122002',
    country: 'INDIA',
...
export default function HssDetails() {
  const [data, setData] = useState<HssRow[]>(initialData);
  const [selectAll, setSelectAll] = useState(false);
  
  // Column Widths
  const checkboxWidth = 40;
  const snoWidth = 60;
  const importerNameWidth = 200;
  ...
  const [editingCell, setEditingCell] = useState<{ rowId: string, columnKey: keyof HssRow } | null>(null);
  ...
  const columns: { key: keyof HssRow | 'checkbox' | 'sNo' | 'actions'; label: string; width: number; required?: boolean; fixed?: 'left' | 'right'; left?: number; right?: number }[] = [
    { key: 'checkbox', label: '', width: checkboxWidth, fixed: 'left', left: 0 },
    { key: 'sNo', label: 'S.No', width: snoWidth, fixed: 'left', left: checkboxWidth },
    { key: 'importerName', label: 'Importer Name', width: importerNameWidth, required: true, fixed: 'left', left: checkboxWidth + snoWidth },
    { key: 'importerId', label: 'ID', width: columnWidths[0], required: true },
    { key: 'iec', label: 'IEC', width: columnWidths[1], required: true },
    { key: 'branch', label: 'Branch', width: columnWidths[2], required: true },
    { key: 'address', label: 'Address', width: columnWidths[3] },
    { key: 'city', label: 'City', width: columnWidths[4] },
    { key: 'state', label: 'State', width: columnWidths[5] },
    { key: 'pincode', label: 'Pincode', width: columnWidths[6] },
    { key: 'country', label: 'Country', width: columnWidths[7] },
    { key: 'adCode', label: 'AD Code', width: columnWidths[8] },
    { key: 'precedingLevel', label: 'Preceding Level', width: columnWidths[9], required: true },
  ...
  const editableKeys: (keyof HssRow)[] = [
    'importerName', 'importerId', 'iec', 'branch', 'address', 
    'city', 'state', 'pincode', 'country', 'adCode', 'precedingLevel'
  ];

  const navigableColumns = columns.filter(c => editableKeys.includes(c.key as keyof HssRow));
  ...
  // Editing Logic
  const startEdit = (rowId: string, columnKey: keyof HssRow, value: any) => {
  ...
  const navigateTo = (rowIndex: number, colIndex: number) => {
    ...
    const cellId = `hss-cell-${r}-${colKey}`;
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (col && data[rowIndex]) startEdit(data[rowIndex].id, col.key as keyof HssRow, data[rowIndex][col.key as keyof HssRow]);
        break;
      case 'Tab':
        e.preventDefault();
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white font-['Inter',sans-serif]" ref={tableRef} onMouseLeave={() => !focusedCellId && setHighlightPosition(null)}>
      {/* Toolbar */}
      <div className="flex items-center justify-center p-3 border-b border-[#d0d5e3] bg-[#DDE3F2] relative">
         <h2 className="text-[16px] font-semibold text-[#050e25]">HSS Details</h2>
      </div>
            ...
            {data.map((row, rIdx) => (
              <div key={row.id} className="flex h-[34px] group relative">
                {columns.map((col, cIdx) => {
                  ...
                  const cellId = `hss-cell-${rIdx}-${col.key}`;
                  ...
                  return (
                    <div 
                      ...
                      onDoubleClick={() => startEdit(row.id, col.key as keyof HssRow, row[col.key as keyof HssRow])}
                    ...
                    >
                      {isEditing ? (
                        ...
                        <>
                          <span className="truncate w-full">{row[col.key as keyof HssRow] as string}</span>
                        </>
                      )}
                    </div>
                  );
           ...
           <div ref={footerScrollRef} className={SCROLLBAR_CLASSES.HIDDEN} onScroll={handleFooterScroll}>
             <div className="flex h-[34px]">
               ...
               <div className="flex sticky left-0 z-30 bg-white shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                 ...
                 <div 
                   style={{ width: importerNameWidth, height: TABLE_CONSTANTS.ROW_HEIGHT }} 
                   className="flex items-center px-2 border-r border-[#d0d5e3] text-[12px] font-medium"
                 >
                   {data.length} items
                 </div>
               </div>
               ...
               {scrollableColumns.map((col, idx) => (
                 <div 
                   key={col.key} 
                   style={{ width: col.width, height: TABLE_CONSTANTS.ROW_HEIGHT }}
                   className="border-r border-[#d0d5e3] flex items-center px-2 bg-white"
                 >
                   {/* Add totals logic here if needed for numeric columns */}
                 </div>
               ))}
